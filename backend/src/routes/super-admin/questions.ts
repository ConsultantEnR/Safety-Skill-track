...
router.get("/template", authenticate, requireRole("SUPER_ADMIN"), async (_req, res, next) => {
  try {
    ...
    ws.columns = [
      ...
      { header: "Compétence",           key: "st1",  width: 22 },
      { header: "Sous-compétence",      key: "st2",  width: 22 },
    ...
    const instructions = [
      ...
      ["  Thématique            Famille (ex. Techniques, Fondamentales)"],
      ["  Sous-thème 1          Compétence principale"],
      ["  Sous-thème 2          Sous-compétence détaillée"],
